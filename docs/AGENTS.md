...
>
> ルートの `AGENTS.md` は要約版です。矛盾がある場合は **このファイルを正** とします。
...
- `internal/domain/` 以外にビジネスロジックを書くこと（API のクリーンアーキテクチャ境界）
- テストなしで実装コードを追加すること
...
- 破壊的変更は PR の説明に明記する
- 人間向けの長文マニュアルは `docs/` に置き、ルートは概要とリンクに留める
...
cli/
  cmd/webapp-cli/  ← CLI エントリ（独立モジュール）
...
- PR は 400 行以内を目標
- レビュー前に `make check`（または `mise run check`）を通す
